<script setup="">
  ...
  import {admin_friends_list} from "../../../http/request";
  ...
  async function getFriendsLists() {
    let newVar = await admin_friends_list({"data": "", "type": "get"});
    console.log(newVar);
...
<!--            <el-button link type="primary" size="small" @click="updateOneUser(scope.$index, scope.row)">Edit</el-button>-->
  ...
  <el-dialog draggable width="750px" :modal="false" :show-close="false" class="publish-comment" align-center v-model="alertDialog" :close-on-click-modal="false">
    <div>
      ...
      <el-input type="text" placeholder="请添加姓名～" v-model="form.from"/>
      <el-input type="text" placeholder="请添加账号～" v-model="form.id"/>
      <el-input type="text" placeholder="请添加姓名～" v-model="form.to"/>
